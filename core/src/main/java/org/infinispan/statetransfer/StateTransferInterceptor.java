...
public class StateTransferInterceptor extends BaseStateTransferInterceptor {
      ...
      return ctx.forkInvocation(command, (ctx1, command1, rv, throwable) -> {
         ...
         if (trace)
            log.tracef("Retrying command because of topology change, current topology is %d: %s",
                  currentTopologyId(), command1);
         FlagAffectedCommand flagAffectedCommand = (FlagAffectedCommand) command1;
         ...
         return visitReadCommand(ctx, flagAffectedCommand, consistentHashUpdater);
      });
   }
   ...
   private CompletableFuture<Void> handleTxCommand(TxInvocationContext ctx, TransactionBoundaryCommand command) throws Throwable {
      ...
      return ctx.forkInvocation(command, (ctx1, command1, rv, throwable) -> {
         ...
         TransactionBoundaryCommand txCommand = (TransactionBoundaryCommand) command1;
         ...
         if (async) {
            stateTransferManager.forwardCommandIfNeeded(txCommand, getAffectedKeys(ctx, txCommand), origin);
            return ctx.shortCircuit(rv);
         }
         ...
         if (ctx.isOriginLocal()) {
            ...
            if (retryTopologyId > 0) {
               ...
               return handleTxCommand(ctx, txCommand);
            }
         } else {
   ...
   private CompletableFuture<Void> handleTxWriteCommand(InvocationContext ctx, WriteCommand command)
      ...
      return ctx.forkInvocation(command, (ctx1, command1, rv, throwable) -> {
         int retryTopologyId = -1;
         WriteCommand writeCommand = (WriteCommand) command1;
         if (throwable instanceof OutdatedTopologyException) {
            // This can only happen on the originator
         ...
         if (ctx.isOriginLocal()) {
            ...
            if (retryTopologyId > 0) {
               ...
               return handleTxWriteCommand(ctx, writeCommand);
            }
         } else {
