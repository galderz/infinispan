...
public class TxInterceptor<K, V> extends DDSequentialInterceptor implements JmxStatisticsExposer {
   ...
   private CompletableFuture<Void> handlePrepareCommand(TxInvocationContext ctx, PrepareCommand command,
      ...
      return ctx.forkInvocation(command, (ctx1, command1, rv, throwable) -> {
         if (!ctx.isOriginLocal()) {
            ForkReturnHandler newContinuation = (ctx2, command2, rv1, throwable1) -> {
               if (throwable == null && !ctx.isOriginLocal()) {
               ...
               return continuation.handle(ctx2, command2, rv1, throwable1);
            };

            return verifyRemoteTransaction((RemoteTxInvocationContext) ctx, command, newContinuation, rv,
                  throwable);
         }

         return continuation.handle(ctx1, command1, rv, throwable);
      });
   }
      ...
      if (!ctx.isOriginLocal()) {
         ...
         if (!isTotalOrder) {
            return replayRemoteTransactionIfNeeded((RemoteTxInvocationContext) ctx, command.getTopologyId(),
                  (ctx1, command1, rv, throwable) -> finishCommit((TxInvocationContext<?>) ctx1));
         }
      }
   ...
   public CompletableFuture<Void> visitLockControlCommand(TxInvocationContext ctx, LockControlCommand command) throws Throwable {
      ...
      return ctx.forkInvocation(command, (ctx1, command1, rv, throwable) -> {
         if (!ctx.isOriginLocal()) {
            return verifyRemoteTransaction((RemoteTxInvocationContext) ctx, command,
                  shortCircuitForkReturnHandler, rv, throwable);
         }
         return shortCircuitOrThrow(ctx1, rv, throwable);
      });
   }
   ...
   public CompletableFuture<Void> visitKeySetCommand(InvocationContext ctx, KeySetCommand command)
      ...
      enlistIfNeeded(ctx);
      return ctx.onReturn((ctx1, command1, rv, throwable) -> {
         if (throwable == null && ctx.isInTxScope()) {
            CacheSet<K> set = (CacheSet<K>) rv;
            return CompletableFuture.completedFuture(
                  new AbstractDelegatingKeyCacheSet(Caches.getCacheWithFlags(cache, command), set) {
                     @Override
                     ...
                     public CloseableSpliterator<K> spliterator() {
                        ...
                        long estimateSize =
                              parentSpliterator.estimateSize() + ctx.getLookedUpEntries().size();
   ...
   private CompletableFuture<Void> handleWriteCommand(InvocationContext ctx, WriteCommand command)
      ...
      return ctx.onReturn((ctx1, command1, rv1, throwable) -> {
         ...
         if (shouldEnlist(ctx) && command.isSuccessful()) {
            TxInvocationContext<LocalTransaction> txContext = (TxInvocationContext<LocalTransaction>) ctx1;
            txContext.getCacheTransaction().addModification(command);
         }
         return null;
      });
   }
   ...
   private CompletableFuture<Void> verifyRemoteTransaction(RemoteTxInvocationContext ctx,
      ...
      if (alreadyCompleted || (originatorMissing && (canRollback || partitionHandlingManager.canRollbackTransactionAfterOriginatorLeave(globalTransaction)))) {
         ...
         return ctx.forkInvocation(rollback, (ctx1, command1, rv1, throwable1) -> {
            ...
            return continuation.handle(ctx1, command1, rv1, throwable1);
         });
      }
