...
public class PartitionHandlingInterceptor extends DDSequentialInterceptor {
   ...
   private CompletableFuture<Void> handleDataReadCommand(InvocationContext ctx, DataCommand command) {
      ...
      return ctx.onReturn((ctx1, command1, rv, throwable) -> {
         if (throwable != null) {
            if (throwable instanceof RpcException && performPartitionCheck(ctx1, ((DataCommand) command1))) {
         ...
         postOperationPartitionCheck(ctx, ((DataCommand) command1), key, rv);
         return null;
      });
   }
   ...
   public CompletableFuture<Void> visitPrepareCommand(TxInvocationContext ctx, PrepareCommand command) throws Throwable {
      if (ctx.isOriginLocal()) {
         return ctx.onReturn((ctx1, command1, rv, throwable) -> {
            if (throwable == null) {
               postTxCommandCheck(((TxInvocationContext) ctx1));
            }
            return null;
         });
      }
   ...
   public CompletableFuture<Void> visitGetAllCommand(InvocationContext ctx, GetAllCommand command) throws Throwable {
      return ctx.onReturn((ctx1, command1, rv, throwable) -> {
         if (throwable != null) {
            if (throwable instanceof RpcException && performPartitionCheck(ctx, command)) {
               // We must have received an AvailabilityException from one of the owners.
               // There is no way to verify the cause here, but there isn't any other way to get an invalid
               // get response.
               throw log.degradedModeKeysUnavailable(((GetAllCommand) command1).getKeys());
            } else {
               throw throwable;
            }
         }
