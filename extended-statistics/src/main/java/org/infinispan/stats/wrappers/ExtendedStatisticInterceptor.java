...
public class ExtendedStatisticInterceptor extends BaseCustomSequentialInterceptor {
   ...
   public CompletableFuture<Void> visitPrepareCommand(TxInvocationContext ctx, PrepareCommand command) throws Throwable {
      ...
      return ctx.onReturn((ctx1, command1, rv, throwable) -> {
         if (throwable != null) {
            processWriteException(ctx, globalTransaction, throwable);
         } else {
            long end = timeService.time();
            updateTime(PREPARE_EXECUTION_TIME, NUM_PREPARE_COMMAND, start, end, globalTransaction, ctx
                  .isOriginLocal());
         }
   ...
   private CompletableFuture<Void> visitWriteCommand(InvocationContext ctx, WriteCommand command, Object key) throws Throwable {
      ...
      return ctx.onReturn((ctx1, command1, rv, throwable) -> {
         ...
         if (throwable != null) {
            processWriteException(ctx1, getGlobalTransaction(ctx1), throwable);
         } else {
            if (isRemote(key)) {
